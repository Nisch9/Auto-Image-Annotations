...
const { predictImage, takeImage } = require('./coco');
...
app.post('/api/mlmodels/:id', (req, res) => {
  ...
  // console.log(req.body["instances"][0]["input_bytes"]["b64"]);
  // request
  //   .post(model.url, {
  //     headers: {
  ...
  const absolutePath = resolve(
    req.body['instances'][0]['input_bytes']['imgurl']
  );
  console.log('path');
  // C:\uploads\1\1.jpg
  // C:\Users\HP\Documents\PS\Model\label-tool\server\uploads\1
  var str = '/Users/nischithadavala/Desktop/PS/label-tool/server/';
  const p = str + absolutePath.substring(1);
  console.log(p);
  takeImage(p).then(predictions => {
  ...
  //   "predictions": [
  //     {
  //       "det_boxes": [0.144, 0.2800, 0.862, 0.75],
  //       "det_class": "car",
  //       "det_score": 0.9
  //     }
  //   ]
  // }
  // predictions.pipe(res);
  // res.send(predictions);
});
